...
async def run(robot: cozmo.robot.Robot):
    ...
    while execute:
        ...
        if not convergence:
            ...
            gui.show_mean(m_x, m_y, m_h, converged)
            ...
            if converged:
                ...
                print('x: {}, y: {}, h: {}'.format(m_x, m_y, m_h))
                # go to goal
                # await robot.turn_in_place(cozmo.util.degrees(diff_heading_deg(0, m_h)), num_retries=2).wait_for_completed()
                # rot_angle = diff_heading_deg(math.degrees(math.atan2(dy, dx)), m_h)
                # await robot.turn_in_place(cozmo.util.degrees(rot_angle), num_retries=2).wait_for_completed()
                # await robot.drive_straight(cozmo.util.distance_inches(math.sqrt(dx**2 + dy**2)), cozmo.util.speed_mmps(30)).wait_for_completed()
                # await robot.turn_in_place(cozmo.util.degrees(diff_heading_deg(goal_h, m_h + rot_angle)), num_retries=2).wait_for_completed()
                # happy
