...
class CozMap:
    ...
    def compute_smooth_path(self, limit=75):
        ...
        path = self.get_path()
        curr_node = path[-1]
        while curr_node != self.get_start():
            parent = curr_node.parent
            go = True
            while go and parent != self.get_start():
                if parent != None and not self.is_collision_with_obstacles((curr_node, parent)):
        ...
        path = self.get_path()
        # segment so that paths are not too long
        for i in range(0, len(path) - 1):
            if get_dist(path[i], path[i+1]) > 75:
                middle_node = rrt.step_from_to(path[i], path[i+1])
                middle_node.parent = path[i]
                path[i+1].parent = middle_node
        return path
