    ...
    while execute:
        ...
        if not convergence:
            ...
            if converged:
                ...
                # go to goal
                rot_angle = diff_heading_deg(math.degrees(math.atan2(dy, dx)), h)
                await robot.turn_in_place(degrees(rot_angle), num_retries=2).wait_for_completed()
                await robot.drive_straight(distance_mm(math.sqrt(dx**2 + dy**2)), speed_mmps(30)).wait_for_completed()
