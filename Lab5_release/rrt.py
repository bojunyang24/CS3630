...
async def CozmoPlanning(robot: cozmo.robot.Robot):
    ...
    goal_node = Node((map_w/2.0, map_h/2.0))
    ...
    while curr_node.x != goal_node.x and curr_node.y != goal_node.y:
        ...
        update_cmap, goal_center, marked = await detect_cube_and_update_cmap(robot, marked, robot.pose.position)
        ...
        if update_cmap:
            ...
            # path = cmap.get_path()
    # set angle back to 0
    await robot.turn_in_place(cozmo.util.radians(-1 * angle_wrt_global), is_absolute=False).wait_for_completed()
    ########################################################################
